...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="코드 문제 평가셋 기준으로 Question-Concept VERIFIED_MENTIONS 연결 정확도를 측정합니다."
    )
    parser.add_argument(
        "--gold",
        default=str(DEFAULT_GOLD_PATH),
        help="평가셋 JSON 경로입니다. 기본값: data/evaluation/question_language_gold.seed.json",
    )
    parser.add_argument(
...
def fetch_predictions(graph: Neo4jGraph, problem_ids: list[str]) -> dict[str, Prediction]:
    # 평가셋 순서를 유지하기 위해 problem_id 목록을 UNWIND로 넘기고, 없는 Question도 같이 확인합니다.
    query = """
        ...
        results.append(
            {
                ...
                "concept_source": case.concept_source,
            }
        )
...
def print_summary(gold_path: Path, include_needs_review: bool, evaluation: dict, show_correct: bool) -> None:
    ...
    print("=== Code Question VERIFIED_MENTIONS baseline ===")
    ...
    for row in details:
        ...
        print(f"  language: {row['expected_language']}")
        print(f"  concept_source: {row['concept_source']}")
...
def main() -> None:
    ...
    if not cases:
        raise SystemExit("평가할 코드 문제 케이스가 없습니다. gold JSON의 review_status를 확인하세요.")
