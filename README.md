...
---
## Question-Concept 자동 연결 성능 개선

기출문제(Question)를 요약 단원(Concept)에 자동 연결할 때, 단순 벡터 유사도만 사용하면 코드형 문제에서 SQL/C/Java/Python 단원이 서로 섞이거나 출제 날짜가 맞지 않는 단원으로 연결되는 문제가 있었습니다.

이를 개선하기 위해 다음 로직을 적용했습니다.

- **날짜 기반 후보 검증**: Question의 실기 출제 날짜와 Concept의 `practical_dates`가 일치하지 않는 후보를 제거
- **Kiwi 기반 BM25 토크나이징**: 조사/어미를 제거하고 명사, 영문 키워드, 코드 식별자를 중심으로 BM25 점수 계산
- **명시 언어 후보군 제한**: 문제에 `SQL`, `C언어`, `Java`, `Python` 등이 직접 등장하면 해당 언어 Concept 범위 안에서만 후보 선택
- **코드 힌트 보정**: `System.out.printf()`처럼 Java 코드가 C의 `printf()`로 오인되지 않도록 정규식 규칙 보정
- **dry-run 평가 스크립트 추가**: Neo4j 관계를 수정하지 않고 연결 전략별 정확도를 비교하는 평가 환경 구성

개선 결과는 다음과 같습니다.

| 평가 항목 | 개선 전 | 개선 후 |
| --- | ---: | ---: |
| 자동 연결 커버리지 | 364/380 (95.8%) | 376/380 (98.95%) |
| 코드형 문제 언어 대분류 정확도 | 85.21% | 100.00% |
| 코드/SQL 세부 단원 연결 정확도 | 53.52% | 64.79% |

세부 단원 연결 정확도는 아직 보완 여지가 있지만, 평가 기준이 단순 언어 분류가 아니라 SQL/C/Java/Python 내부의 세부 단원까지 맞히는 기준이므로 난이도가 높습니다. 현재는 자동 연결 커버리지와 언어 대분류 오연결 제거를 우선 개선했고, 남은 세부 단원 오연결은 추후 수동 override 파일로 보정할 수 있도록 평가셋과 dry-run 비교 스크립트를 분리했습니다.
